...
Facnet_config = {
    ...
    "learning_rate": 10**-3,       # Learning rate for optimizer
...
def forward_all_images(model,device,all_img):
    ...
    all_img     = torch.from_numpy(all_img.cpu().numpy())
...
def distillation_loss(student_output, teacher_output, labels, T=2.0, alpha=0.7):
    # KL divergence between teacher and student output for old classes
    ...
    # Combine distillation and cross-entropy loss
...
def learning_function(model, old_model, optimizer, device, images_train, labels_train, images_test, labels_test, increment):
    ...
    # Training and centroid update happens here as described before
    ...
    for i in range(1,Epochs+1):
        ...
        for s in range(clusters_centers.shape[0]):
            ...
            targets = torch.from_numpy(targets.cpu().numpy()).to(device=device, dtype=torch.float)
            ...
            loss = loss_function(Facnet_config['alpha']).forward(output, batch_label, chosen_clusters, device)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            with torch.no_grad():
                old_outputs = old_model(images_train)

            # Compute distillation loss
            loss = distillation_loss(outputs, old_outputs, labels_train)
            loss.backward()
            optimizer.step()
        ...
        train_acc.append(train_acc_1)
        test_acc.append(test_acc_1)
        print(f"First 20 labels in testing data: {labels_test[:20]}")
        print(f"First 20 labels in testing data: {labels_train[:20]}")
        torch.save(model.state_dict(), r'D:\Deep-metric\continual_learning\models'+str(i)+'-'+str(increment)+'.pth')
    ...
    np.save(r'D:\Deep-metric\continual_learning\data'+str(increment),train_acc)
    np.save(r'D:\Deep-metric\continual_learning\data'+str(increment),test_acc)
...
class EWC:
    ...
    def compute_fisher_information(self, dataloader):
        fisher_information = {n: torch.zeros_like(p) for n, p in self.params.items()}
        self.model.eval()
    ...
    def penalty(self):
        ...
        for n, p in self.model.named_parameters():
            if p.requires_grad:
                ...
                loss += (fisher * (p - self.params[n]) ** 2).sum()
...
# Incremental training over 5 increments (each increment = 2 classes)
for increment in range(num_sets):
    ...
    if increment > 0:
        ...
        old_model = load_pretrained_model(f'D:\Deep-metric\continual_learning\models_'+str(increment)+'.pth', old_model, device)
    else:
        old_model = WRN2(width)  # First time, no distillation from old model
    # Train on the current set
    current_train_indices = train_indices[increment]
    current_train_loader = DataLoader(
        Subset(train_dataset, current_train_indices.astype(int).tolist()), batch_size=64, shuffle=True
    )

    # Evaluate on the accumulated test sets so far
    accumulated_test_indices = np.concatenate([accumulated_test_indices, test_indices[increment]])

    current_test_loader = DataLoader(
        Subset(test_dataset, accumulated_test_indices.astype(int).tolist()), batch_size=64, shuffle=False
    )

    # Train the model on the current increment
    images_train, labels_train = get_images_and_labels(current_train_loader)
    images_test, labels_test = get_images_and_labels(current_test_loader)
    print(f"Number of training samples: {len(images_train)}, Labels: {len(labels_train)}")

    # Train model with distillation
    learning_function(model, old_model, optimizer, device, images_train, labels_train, images_test, labels_test, increment)
